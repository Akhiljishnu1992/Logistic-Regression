setwd("~/Desktop/Edvancer/Data/Data")
...
tb = table(test$target,test$predicted)
tb
tb[1,1]
tb[1,2]
tb[2,1]
...
tb = table(test$target,target.tree)
# Performance
...
rf=randomForest(target~.,data=train,do.trace=F)

test.rf=predict(rf,newdata=test)

varImpPlot(rf)

tb = table(test$target,test.rf)
